      ...
      <div className="flex flex-wrap justify-center gap-4 mt-6">
        {slides.map((_, idx) => (
          <button key={idx} onClick={() => { setPaused(true); setActiveIndex(idx) }}
            className={`rounded-full w-[20px] h-[20px] transition-[background] hover:bg-teal-700 ${idx === activeIndex ? 'bg-teal-100' : 'bg-teal-500'}`}></button>
        ))}
      </div>
      <div className="flex gap-4 my-6">
        <button className="drop-shadow bg-teal-500 p-4 hover:bg-teal-600 transition-[background] rounded-lg" onClick={() => { setPaused(true); alterIndex(-1) }}>
          Previous
        </button>
        <button className="drop-shadow bg-teal-500 p-4 hover:bg-teal-600 transition-[background] rounded-lg" onClick={() => { setPaused(true); alterIndex(+1) }}>
          Next
